...
Note that base R's `set.seed()` has no effect on `mirai` @mirai. Further, note that by default if `daemons()` is called without any arguments, the RNG streams aren't reproducible (just like results in serial execution without setting a seed). I.e., running `rexp()` again will get different results:
