...
> Flo: Explanation of what the mirai code is doing would help to understand the behavior. When I understand it correctly then `daemons(4)` starts 4 background worker processes (cf. daemons). `mirai_map(1:4, rexp)` dispatches 4 tasks (`rexp(1), rexp(2), rexp(3), rexp(4)`) to the 4 daemons. The `[]` blocks the main R session until all 4 tasks have completed, then returns a list containing all the results. Finally, `mirai::daemons(0)` shuts down and resets all mirai daemons for the current compute profile.
>
> Regarding seed: setting the seed via `set.seed(0)` will set the RNG seed in the host R sessions, but **not** in each daemon.
...
> Flo: Here the number of daemons is reduced to 2, thus 2 R sessions are started. The first 2 tasks (`rexp(1), ...`) are dispatched to the 2 daemons and whenever a daemon finishes the task the next task is allocated to it.
>
> Regarding seed: As the seed is related to the session, only the first two sessions produce the same results as above.
...
Setting a seed manually when launching daemons gets reproducibility (stability over runs), "regardless of the number of daemons used." @mirai-2.5.0.

> Flo: Instead of setting seed for the host session, we can set a seed for the daemons explicitly, which initializes a stream per mirai task (instead of host session)
