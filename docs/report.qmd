...
> Flo:
> Probably here just super short the content of the GSoC project and a short
> outline (can also be added later). No need to go too much into the 
> background (e.g., what is parallelisation).
...
# Approaches to parallelisation {#sec-approaches-to-parallelisation}

> Flo:
> This is kinda "state of the art" section
> I think a short explanation of special terms is helpful to understand the
> differences in diverse approaches (e.g., serial execution, forking, 
> synchronous, asynchronous etc.)
> I started with a `glossary.qmd` that we could use to collect and define
> these terms.
...
3. Fresh processes through `parallel::parLapply()`
  ...
  + parallel, synchronous
4. Parallelisation with `mirai_map()` (**new**)
  ...
  + allows for asynchronous parallelisation with persistent workers

**Comparison between approaches**

| | `lapply()` | `mclapply()` | `parLapply()` | `mirai_map()[]` | `mirai_map()` |
|---|---|---|---|---|---|
| Parallel | no | yes | yes | yes | yes |
| Synchronous | yes | yes | yes | yes | no |
| Asynchronous | no | no | no | no | yes |
| Mechanism | host session | fork | PSOCK socket | NNG daemon | NNG daemon |
| Shared memory | n/a | yes | no | no | no |
| Windows | yes | no | yes | yes | yes |
| Persistent workers | no | no | yes | yes | yes |
| Export data | no | no | yes | yes | yes |
| Setup needed | none | none | `makeCluster` | `daemons()` | `daemons()` |

Thus, `mirai` works for all OS and allows for asynchronous parallelisation.
Only caveat, it does not provide a possibility for shared memory (but for this `mori` has been developed that complements `mirai` for solving this issue (see Section )).

> Flo:
> I think now the motivation becomes clearer, why we are interested in mirai
...
> Flo:
> Explanation of what the mirai code is doing would help to understand the
> behavior. When I understand it correctly then `daemons(4)` starts 4
> background worker processes (cf. daemons). `mirai_map(1:4, rexp)` dispatches 4 
> tasks (`rexp(1), rexp(2), rexp(3), rexp(4)`) to the 4 daemons. The `[]`
> blocks the main R session until all 4 tasks have completed, then returns a 
> list containing all the results. Finally, `mirai::daemons(0)` shuts down and 
> resets all mirai daemons for the current compute profile.
>
> Regarding seed: setting the seed via `set.seed(0)` will set the RNG seed in 
> the host R sessions, but **not** in each daemon.
```{r}
set.seed(0)
str(lapply(1:4, rexp))
...
However, changing the number of daemons does not maintain the same results, which is obviously problematic:

> Flo:
> Here the number of daemons is reduced to 2, thus 2 R sessions are started.
> The first 2 tasks (`rexp(1), ...`) are dispatched to the 2 daemons and 
> whenever a daemon finishes the task the next task is allocated to it. 
>
> Regarding seed:
> As the seed is related to the session, only the first two sessions produce
...
Setting a seed manually when launching daemons gets reproducibility (stabilty over runs), "regardless of the number of daemons used." @mirai-2.5.0.

> Flo:
> Instead of setting seed for the host session, we can set a seed for the 
> daemons explicitly, which initializes a stream per mirai task (instead of
> host session) 
...
`mirai` by itself does not allow for shared memory. However, [`mori`](https://shikokuchuo.net/mori/) is a recently released R packages that allows for shared memory by placing an R object once into OS-level shared memory and letting every process on the machine read the same physical pages directly, with no data copying between processes ([source](https://opensource.posit.co/blog/2026-04-23_mori-0-1-0/)).
...
When browsing the codebase to find places where parallelization could be helpful (see @sec-q4-parallelization-benefit), we found two potential speedups--in these places, we may be able to benefit from _vectorizing_ functions instead of parallelizing a loop. Vectorization is advantageous in these spots as we would be rewriting R loops as optimized matrix operations [@advr, secs. 24.5-24.7].
